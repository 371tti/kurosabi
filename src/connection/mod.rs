...
impl<C, R: AsyncRead + Unpin + 'static, W: AsyncWrite + Unpin + 'static> Connection<C, R, W, StatusSetNoneBody> {
    ...
    #[inline]
    pub async fn streaming_unchunked<T>(
        mut self,
        mut reader: T,
        size: u64,
    ) -> ConnectionResult<Connection<C, R, W, ResponseReadyToSend>>
                    ...
                    Some(r) => {
                        match parse_range_header_value(r) {
                            RangeParse::Valid(RangeSpec::FromToInclusive { start, end }) => {
                                ...
                                let end = (end.saturating_add(1)).min(range.end).min(start.saturating_add(file_content.max_size));
                                if start < end {
                                    start..end
                                } else {
                                    return Ok(self
                            ...
                            RangeParse::Valid(RangeSpec::Suffix { len }) => {
                                let range = file_content.default_range;
                                let end = range.end;
                                let start = end.saturating_sub(len.min(file_content.max_size)).max(range.start);
                                if start < end {
                                    start..end
                                } else {
                                    return Ok(self
